...
空结果常见原因：
  - 网易严选搜索页为前端 JS 渲染，静态 HTML 仅约 8KB 壳，不含商品列表。
  - 真实商品数据经内部接口异步拉取，需登录态 / 风控参数。
  - 未满足渲染条件时静态抓取返回 []，属预期。

生产建议：配置 PC_COOKIE_YANXUAN 或改用 Playwright 渲染。抓取失败一律返回空列表。
"""
import os
import re
import urllib.parse
    ...
    def _parse(html: str, keyword: str) -> List[Product]:
        ...
        for i, price in enumerate(prices[:50]):
            try:
                ...
                url = ""
                if i < len(ids):
                    url = "https://you.163.com/item/detail?id=" + str(ids[i])
